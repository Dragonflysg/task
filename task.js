$(document).ready(function () {
    ...
    function handleIncomingPatch(data) {
        ...
        // Handle grid cell patches from grid.html
    ...
    function buildSubtaskPredPicker(subtaskId, predArray) {
        ...
        $area.append($input);
        $picker.append($area);
        $picker.append($('<button type="button" class="pp-dropdown-btn">').html('<i class="fa fa-caret-down"></i>'));
    ...
    function renderSubtaskTable(parentTask) {
        ...
        $.each(allRows, function (idx, row) {
            ...
            if (row.siblingCount > 1) {
                ...
                if (!row.isFirst) {
                    $arrows.append(
                        $('<button class="subtask-reorder-btn" title="Move up">')
                ...
                if (!row.isLast) {
                    $arrows.append(
                        $('<button class="subtask-reorder-btn" title="Move down">')
                            .attr('data-reorder-id', st.id)
            ...
            if (depth < 2 && st.name && st.name.trim() !== '') {
                $nameWrap.append(
                    $('<button class="row-action-btn btn-add-child-inline" title="Add sub-subtask">')
            ...
            // % Complete
            $tr.append($('<td>').append(
                $('<input type="text">').val(st.percentComplete).attr('data-field', 'percentComplete').attr('data-id', st.id).css('text-align', 'center')
            ));
    ...
    $('#btn-add-subtask').on('click', function () {
        ...
        saveData();
        sendPatch({op: 'addSubtask', parentTaskId: task.id, subtask: JSON.parse(JSON.stringify(sub))});
        renderDetail();
        setTimeout(function () {
    ...
    $(document).on('click', '[data-add-child]', function () {
        ...
        saveData();
        sendPatch({op: 'addSubtask', parentTaskId: parentSubId, subtask: JSON.parse(JSON.stringify(child))});
        renderDetail();
        setTimeout(function () {
            var $inputs = $('#subtask-body input[data-field="name"]');
            $inputs.last().focus();
    ...
    $(document).on('input change blur', '#subtask-body input[data-field], #subtask-body select[data-field]', function (e) {
        ...
        if (field === 'duration') {
            ...
            var start = new Date(sub.startDate + 'T00:00:00');
            var end = new Date(start.getTime() + days * 24 * 60 * 60 * 1000);
            var iso = end.toISOString().slice(0, 10);
            sub.endDate = iso;
        ...
        if (field === 'name' || field === 'cost' || field === 'percentComplete') {
        ...
        if (field === 'name' && e.type === 'input') {
            ...
            // Only show add-child for depth-0 and depth-1 subtasks
    ...
    // ========== Full Save (sync cellData for grid view) ==========
    $('#btn-save-all').on('click', function () {
        ...
        // Load latest task data from server (includes all users' patches),
        // then rebuild the full save structure (including cellData for grid)
        $.ajax({
            url: '/api/load-group',
            method: 'GET',
            ...
            success: function (resp) {
                if (resp && resp.ok && resp.data && resp.data._taskData) {
                    // Use server's latest task data (has all patches from all users)
                ...
                // Rebuild full structure and save (no version check)
                var payload = {
                    project: PROJ_NAME,
                    data: buildSaveData()
                };
            ...
            error: function () {
                // If can't load latest, save with local data anyway
                var payload = {
                    project: PROJ_NAME,
                    data: buildSaveData()
                };
