$(document).ready(function () {
    ...
    // ========== Predecessor badge + timeline helpers (ported from design.js) ==========
    var predColors = [
        { bg: '#e8edf4', text: '#3a5a8c', border: '#c8d4e8' },
        { bg: '#fce8db', text: '#9a4e2a', border: '#f0cdb8' },
        { bg: '#ede0f5', text: '#6b3fa0', border: '#d8c2ea' },
        { bg: '#fde8e8', text: '#a83232', border: '#f4c4c4' },
        { bg: '#e0f0f5', text: '#2a6e7a', border: '#b8dce5' },
        { bg: '#f5eacc', text: '#7a6020', border: '#e6d49a' },
        { bg: '#e0e8f8', text: '#3050a0', border: '#b8c8e8' },
        { bg: '#f5e0ea', text: '#8a2a5a', border: '#e4b8cc' }
    ];
    function getPredColor(index) { return predColors[index % predColors.length]; }
    ...
    function getEffectiveStartDate(taskObj, visited) {
        if (!visited) visited = {};
        if (!taskObj) return '';
        if (visited[taskObj.id]) return taskObj.startDate || '';
        visited[taskObj.id] = true;
        ensurePredecessorArray(taskObj);
        if (!taskObj.predecessor || taskObj.predecessor.length === 0) return taskObj.startDate || '';
        var maxEnd = '';
        for (var i = 0; i < taskObj.predecessor.length; i++) {
            ...
            if (!pe) {
                ...
                if (pd > 0) {
                    var es = getEffectiveStartDate(pred, visited);
                    if (es) pe = addWorkingDays(es, pd);
                }
            }
            if (pe && pe > maxEnd) maxEnd = pe;
        }
        return maxEnd ? nextWorkingDay(maxEnd) : (taskObj.startDate || '');
    }
    ...
    function buildEndDateBadges(taskObj) {
        ...
        var $row = $('<div class="edb-row edb-row-combined">');
    ...
    function buildTimelineData(taskObj) {
        ...
        if (data.latestPredEnd) data.effectiveStart = nextWorkingDay(data.latestPredEnd);
        else data.effectiveStart = taskObj.startDate || '';
        ...
        if (data.predecessors.length === 0 && !data.taskEndDate) {
            ...
            for (var pi = 0; pi < ordered.length; pi++) {
                ...
                var $bar = $('<div class="timeline-bar timeline-bar-pred">')
                    .css({ background: pr.color.bg, borderColor: pr.color.border, color: pr.color.text });
    ...
    // ========== Card Order (per-user, per-project, per-column) ==========
    function getCardOrderKey() {
        return 'kanban_card_order_' + PROJ_NAME + '_' + CURRENT_USER_ID;
    }

    function loadCardOrder() {
        try {
            var raw = localStorage.getItem(getCardOrderKey());
            return raw ? JSON.parse(raw) : {};
        } catch (e) { return {}; }
    }

    function saveCardOrder(allOrders) {
        try {
            localStorage.setItem(getCardOrderKey(), JSON.stringify(allOrders));
        } catch (e) {}
    }

    function sortByBreadcrumb(a, b) {
        return a.breadcrumb.join(' > ').toLowerCase().localeCompare(b.breadcrumb.join(' > ').toLowerCase());
    }

    function applyCardOrder(colKey, items) {
        var allOrders = loadCardOrder();
        var savedIds = allOrders[colKey];
        if (!savedIds || savedIds.length === 0) {
            // No saved order — sort alphabetically
            items.sort(sortByBreadcrumb);
            return items;
        }
        // Build lookup by task id
        var byId = {};
        for (var i = 0; i < items.length; i++) {
            byId[items[i].task.id] = items[i];
        }
        var ordered = [];
        // Place items in saved order
        for (var j = 0; j < savedIds.length; j++) {
            if (byId[savedIds[j]]) {
                ordered.push(byId[savedIds[j]]);
                delete byId[savedIds[j]];
            }
        }
        // Append any new items not in saved order (alphabetically)
        var remaining = [];
        for (var id in byId) {
            remaining.push(byId[id]);
        }
        remaining.sort(sortByBreadcrumb);
        return ordered.concat(remaining);
    }

    function saveColumnOrder(colKey) {
        var allOrders = loadCardOrder();
        var ids = [];
        $('.kanban-col-body[data-col="' + colKey + '"] .kanban-card').each(function () {
            ids.push($(this).data('id'));
        });
        allOrders[colKey] = ids;
        saveCardOrder(allOrders);
    }

    function clearColumnOrder(colKey) {
        var allOrders = loadCardOrder();
        delete allOrders[colKey];
        saveCardOrder(allOrders);
    }
    ...
    function renderBoard() {
        ...
        // Apply saved custom order or fall back to alphabetical
        cols['not-started'] = applyCardOrder('not-started', cols['not-started']);
        cols['in-progress'] = applyCardOrder('in-progress', cols['in-progress']);
        cols['done'] = applyCardOrder('done', cols['done']);
    ...
    function buildKanbanCard(item, colKey) {
        ...
        // Flag toggle
        var flagActive = task.flagged ? ' flag-active' : '';
        $footer.append('<span class="kanban-flag-toggle' + flagActive + '" data-id="' + task.id + '" title="Flag for attention"><i class="fa fa-flag"></i></span>');
    ...
    function initDragEvents() {
        ...
        $(document).on('dragend', '.kanban-card.draggable', function () {
            ...
            $('.kanban-card').removeClass('drop-above drop-below');
            draggedId = null;
        });

        // Show drop indicator between cards for same-column reorder
        $(document).on('dragover', '.kanban-col-body', function (e) {
            e.preventDefault();
            e.originalEvent.dataTransfer.dropEffect = 'move';
            var targetCol = $(this).data('col');
            var task = draggedId ? findTaskById(draggedId, tasks) : null;
            var currentCol = task ? (STATUS_TO_COL[task.status] || 'not-started') : null;

            if (currentCol === targetCol) {
                // Same column — show insert indicator on nearest card
                $(this).removeClass('drag-over');
                var $cards = $(this).children('.kanban-card').not('.dragging');
                $cards.removeClass('drop-above drop-below');
                ...
                $cards.each(function () {
                    ...
                    if (!inserted && y < mid) {
                        $(this).addClass('drop-above');
                        inserted = true;
                    }
                });
                if (!inserted && $cards.length > 0) {
                    $cards.last().addClass('drop-below');
                }
            } else {
                // Different column — highlight column
        ...
        $(document).on('dragleave', '.kanban-col-body', function (e) {
            if (!this.contains(e.relatedTarget)) {
                $(this).removeClass('drag-over');
                $(this).find('.kanban-card').removeClass('drop-above drop-below');
            }
        });
        ...
        $(document).on('drop', '.kanban-col-body', function (e) {
            ...
            if (currentCol === targetCol) {
                // Same-column reorder
                var $body = $(this);
                var $draggedCard = $body.children('.kanban-card[data-id="' + draggedId + '"]');
                var y = e.originalEvent.clientY;
                var $cards = $body.children('.kanban-card').not($draggedCard);
                var inserted = false;
                ...
                if (!inserted) {
                    $body.append($draggedCard);
                }

                // Save the new order
                saveColumnOrder(targetCol);
                highlightCard(draggedId);
                return;
            }
            ...
            // Save % Complete to sessionStorage when leaving In Progress
            if (currentCol === 'in-progress') {
                try { sessionStorage.setItem('pct_' + task.id, String(task.percentComplete || 0)); } catch (e) {}
            }

            task.status = newStatus;

            // Set percentComplete based on target column
            if (newStatus === 'Completed') {
                task.percentComplete = 100;
            } else if (newStatus === 'Not Started') {
                task.percentComplete = 0;
            } else if (targetCol === 'in-progress') {
                // Restore saved % Complete if available
                try {
                    var saved = sessionStorage.getItem('pct_' + task.id);
                    if (saved !== null) {
                        task.percentComplete = parseInt(saved) || 0;
                    }
                } catch (e) {}
            }
            ...
            // Remove card from source column's saved order
            var allOrders = loadCardOrder();
            if (allOrders[currentCol]) {
                allOrders[currentCol] = allOrders[currentCol].filter(function (id) { return id !== task.id; });
                saveCardOrder(allOrders);
            }
    ...
    function openModal(taskId) {
        ...
        // End Date (auto-computed for Execution projects; disabled for editing)
        ...
        var endAutoComputed = isExecutionProject();
        var endReadonlyAttr = (!canEdit || endAutoComputed) ? ' readonly' : '';
        var $endInput = $('<input type="text" class="modal-input" data-field="endDate" data-id="' + task.id + '" placeholder="mm/dd/yyyy"' + endReadonlyAttr + '>').val(toDisplayDate(task.endDate));
        if (canEdit && !endAutoComputed) setupDateAutoFormat($endInput);
        ...
        if (!canEdit || endAutoComputed) $endBtn.prop('disabled', true);
        ...
        if (isExecutionProject() && (!task.subtasks || task.subtasks.length === 0)) {
            var $badgesWrap = $('<div class="end-date-badges header-end-date-badges">');
            var $built = buildEndDateBadges(task);
            $badgesWrap.append($built.children());
            if ($badgesWrap.children().length > 0) {
                $predGroup.append($badgesWrap);
            }
        }
    ...
    $(document).on('input change blur', '#modal-body .modal-input', function (e) {
        ...
        if (field === 'duration') {
            ...
            var endIso = addWorkingDays(task.startDate, days);
            task.endDate = endIso;
    ...
    // Flag toggle on card
    $(document).on('click', '.kanban-flag-toggle', function (e) {
        e.stopPropagation();
        var taskId = parseInt($(this).data('id'));
        var task = findTaskById(taskId, tasks);
        if (!task) return;
        task.flagged = !task.flagged;
        $(this).toggleClass('flag-active', task.flagged);
        saveData();
        sendPatch({ op: 'update', taskId: taskId, field: 'flagged', value: task.flagged });
    });

    $(document).on('click', '.kanban-card', function (e) {
        if ($(e.target).closest('.done-status-select').length) return;
        if ($(e.target).closest('.kanban-flag-toggle').length) return;
        var taskId = parseInt($(this).data('id'));
        openModal(taskId);
    });
    ...
    // Sort alphabetically button — clears custom order for that column
    $(document).on('click', '.btn-sort-alpha', function (e) {
        e.stopPropagation();
        var colKey = $(this).data('col');
        clearColumnOrder(colKey);
        renderBoard();
    });
