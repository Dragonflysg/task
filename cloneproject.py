...
def main():
    ...
    # Clear corresponding fields in _taskData (authoritative source that
    # syncCellDataFromTaskData() uses to overwrite cellData on load)
    TASKDATA_CLEAR_FIELDS = {
    ...
    fields_to_clear = [TASKDATA_CLEAR_FIELDS[c] for c in CLEAR_COLUMNS if c in TASKDATA_CLEAR_FIELDS]

    def clear_task_fields(task):
        for field in fields_to_clear:
            if field in task:
                if isinstance(task[field], list):
                    task[field] = []
                elif isinstance(task[field], str):
                    task[field] = ''
                else:
                    task[field] = 0
        for sub in task.get('subtasks', []):
            clear_task_fields(sub)
