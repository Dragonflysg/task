$(document).ready(function () {
    ...
    var currentModalRequestId = null;
    ...
    function deleteRequest(requestId) {
        $.ajax({
            url: '/api/change-requests/delete',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                project: PROJ_NAME,
            ...
            success: function (resp) {
                if (resp.ok) {
                    requests = requests.filter(function (r) { return r.id !== requestId; });
                    renderBoard();
                    showToast('Request deleted');
                } else {
                    showToast('Failed to delete: ' + (resp.error || ''), true);
                }
            },
            error: function () {
                showToast('Server error deleting request', true);
            }
        });
    }
    ...
    $(document).on('keydown', function (e) {
        if (e.key === 'Escape') {
            if ($('#delete-confirm-overlay').is(':visible')) {
    ...
    $('#delete-confirm').on('click', function () {
        if (!currentModalRequestId) return;
        var idToDelete = currentModalRequestId;
        $('#delete-confirm-overlay').hide();
        closeModal();
