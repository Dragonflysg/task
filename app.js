$(document).ready(function () {
    ...
    var preservedTaskData = null;  // _taskData from task.html, preserved across grid saves
    ...
    function sendCellPatch(key, cell) {
        if (!PROJ_NAME || !isGroupProject || isProtectedView) return;
        var patch = {
            op: 'updateCell',
            project: PROJ_NAME,
    ...
    function handleIncomingGridPatch(data) {
        ...
        // Handle task-level patches from task.html
        if (data.op === 'update') {
            ...
            if (field === 'startDate' || field === 'endDate') {
                ...
                if ($durTd.length) renderSingleCell($durTd, row, 3);
            }
            return;
        }
    ...
    // Rebuild _taskData from current cellData so task.html sees grid edits
    ...
    function loadState(data) {
        ...
        // Sync cellData text values from _taskData (task.html patches update
    ...
    function renderSingleCell($td, row, col) {
        ...
        if (colType === 'checkbox') {
            ...
            if (colObj.durationStartCol !== undefined && colObj.durationEndCol !== undefined &&
                ...
                if (startColIdx >= 0 && endColIdx >= 0) {
                    ...
                    if (startDateStr && endDateStr) {
                        ...
                        var endDate = new Date(endDateStr);
                        if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
                            durationText = Math.round((endDate - startDate) / (1000 * 60 * 60 * 24)) + 'd';
                        }
                    }
                }
            }
            ...
            $td.css('position', 'relative');
            var displayDate = data.text ? formatDateDisplay(data.text) : '';
            $td.append($('<span class="cell-content">').text(displayDate));
            $td.append($('<button class="date-cell-icon" data-date-row="' + row + '" data-date-col="' + col + '">').html('<i class="fa fa-calendar-days"></i>'));
            $td.append($('<input type="date" class="date-cell-hidden-input" data-date-row="' + row + '" data-date-col="' + col + '">').val(data.text || ''));
        } else if (colType === 'contacts') {
            renderContactsCell($td, data.text);
        } else if (colType === 'predecessor') {
            renderPredecessorCell($td, data.text);
    ...
    $(document).on('change', '.date-cell-hidden-input', function () {
        ...
        selectCell(row, col);
        sendCellPatch(key, cellData[key]);
        // Also re-patch the duration column if start or end date changed
        for (var ci = 0; ci < columns.length; ci++) {
            if (columns[ci].type === 'duration') {
                var durKey = row + '-' + ci;
                var $durTd = $('td.cell[data-row="' + row + '"][data-col="' + ci + '"]');
                if ($durTd.length) {
