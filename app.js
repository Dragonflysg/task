$(document).ready(function () {
    ...
    function loadDefaultGroupProject() {
        ...
        $('#project-name-label').text(DEFAULT_GROUP_PROJECT);
        initProtectedView();
    ...
    function syncTaskFromRow(taskObj, rowIdx) {
        ...
        // Col 4: Predecessor — grid stores as label (e.g. "CFO > sub1"), _taskData stores as ID
        var predCell = cell(4);
        if (predCell === '' || predCell === undefined) {
            taskObj.predecessor = '';
        } else {
            // Reverse-lookup: find the task ID whose label matches the cell text
            var foundId = reversePredecessorLookup(predCell);
            if (foundId !== null) {
                taskObj.predecessor = foundId;
            }
            // If no match found, keep existing value (label may not have changed)
        }
