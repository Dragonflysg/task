$(document).ready(function () {
    ...
    function syncTaskFromRow(taskObj, rowIdx) {
        ...
        // Col 4: Predecessor — stored as readable label in grid, keep raw value from _taskData
        // unless the cell text changed
        var predCell = cell(4);
        if (predCell !== '' && taskObj.predecessor) {
            // Predecessor is stored as structured data in _taskData, keep as-is
            // unless the cell was cleared
        } else if (predCell === '' && cellData[rowIdx + '-4'] !== undefined) {
            taskObj.predecessor = null;
        }
