$(document).ready(function () {
    ...
    function renderDetail() {
        ...
        $('#detail-duration').val((task._workingDays && task._workingDays > 0) ? task._workingDays + 'd' : calcDuration(task.startDate, task.endDate));
        ...
        if (isExecutionProject()) {
            ...
            // Start Date: disabled unless explicitly enabled
    ...
    function renderSubtaskTable(parentTask) {
        ...
        $.each(allRows, function (idx, row) {
            ...
            var durText = (st._workingDays && st._workingDays > 0) ? st._workingDays + 'd' : calcDuration(st.startDate, st.endDate);
    ...
    $('#detail-duration').on('change blur', function () {
        ...
        var effStart = isExecutionProject() ? getEffectiveStartDate(task) : (task.startDate || '');
        if (!effStart) {
    ...
    $(document).on('input change blur', '#subtask-body input[data-field], #subtask-body select[data-field]', function (e) {
        ...
        if (field === 'duration') {
            ...
            if (isNaN(days) || days < 0) return;
            var effStart = isExecutionProject() ? getEffectiveStartDate(sub) : (sub.startDate || '');
            if (!effStart) {
                alert('Indicate the Start Date first.');
    ...
    // Show the button for all Execution projects; adapt label based on state
    function updateEnableStartDateBtn() {
        var task = findTaskById(selectedTaskId, tasks);
        var $btn = $('#btn-enable-start-date');
        if (isExecutionProject() && task) {
            if (isStartDateEnabled(task)) {
                $btn.html('<i class="fa fa-calendar-check"></i> Change Start Date');
            } else {
                $btn.html('<i class="fa fa-calendar-check"></i> Enable Start Date');
            }
            $btn.show();
        } else {
            $btn.hide();
    ...
    $('#btn-enable-start-date').on('click', function () {
        ...
        var task = findTaskById(selectedTaskId, tasks);
        var enabled = task && isStartDateEnabled(task);
        ...
        if (enabled) {
            $('#esd-dialog-title-text').text('Change Start Date');
            $('#esd-explanation-disabled').hide();
            $('#esd-explanation-enabled').show();
            $dlg.find('.esd-option-reset').show();
            $('#esd-reset-task-name').text(task.name || 'Untitled');
        } else {
    ...
    $('#esd-apply').on('click', function () {
        ...
        if (choice === 'C') {
            // Reset: clear all start dates and end dates, and disable
            task.startDate = '';
            task.endDate = '';
        ...
        if (choice === 'B') {
            ...
            var bulkIso = toIsoDate(bulkDisplay);
            if (!bulkIso) {
                showToast('Please enter a valid date for Option B.');
                return;
            }
            // Set start date on parent task and all subtasks
            task.startDate = bulkIso;
            setStartDateOnAllSubtasks(task.subtasks, bulkIso);
        }

        // Mark start dates as enabled for this task
        task._startDateEnabled = true;
        saveData();
        sendPatch({ op: 'update', taskId: task.id, field: '_startDateEnabled', value: true });
        if (choice === 'B') {
            sendPatch({ op: 'update', taskId: task.id, field: 'startDate', value: task.startDate });
            // Broadcast subtask start dates
            var allRows = [];
            flattenSubtasks(task.subtasks, 0, allRows, task.id);
            for (var i = 0; i < allRows.length; i++) {
