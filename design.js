$(document).ready(function () {
    ...
    function showAllCompleteNotice(taskName, cancelledCount) {
        ...
        if (cancelledCount && cancelledCount > 0) {
            var word = cancelledCount === 1 ? 'subtask' : 'subtasks';
            var verb = cancelledCount === 1 ? 'was' : 'were';
            $box.append(
                $('<p style="font-size:12px;color:#8a6d3b;background:#fcf8e3;border:1px solid #faebcc;border-radius:4px;padding:8px 12px;line-height:1.5;margin:0 0 20px;">').text(
                    'Please note that there ' + verb + ' ' + cancelledCount + ' ' + word + ' that ' + verb + ' set to Cancelled.'
                )
            );
        }
    ...
    function computePercentFromChildren(node) {
        ...
        for (var i = 0; i < node.subtasks.length; i++) {
            if (node.subtasks[i].status === 'Cancelled') continue;
            allCancelled = false;
        ...
        if (allCancelled || count === 0) {
            ...
            node.percentComplete = Math.round(sum / count);
        }
    }
    ...
    function computeStatusFromChildren(node) {
        ...
        } else if (considered > 0 && allCompleted) {
            node.status = 'Completed';
        } else if (considered > 0 && allNotStarted) {
    ...
    function countCancelledDescendants(node) {
        if (!node || !node.subtasks || node.subtasks.length === 0) return 0;
        var n = 0;
        for (var i = 0; i < node.subtasks.length; i++) {
            var child = node.subtasks[i];
            if (child.status === 'Cancelled') n++;
            n += countCancelledDescendants(child);
        }
        return n;
    }
    ...
    function recomputeStatus() {
        ...
        if (task.status === 'Completed' && oldStatus !== 'Completed') {
            ...
            showAllCompleteNotice(taskName, countCancelledDescendants(task));
        }
    }
    ...
    function openTimelineInfoDialog(taskObj) {
        ...
        var $xClose = $('<button class="timeline-dialog-close" title="Close" aria-label="Close"><i class="fa fa-xmark"></i></button>');
        $xClose.on('click', function () { closeDialog(); });
        $box.append($xClose);
        ...
        function closeDialog() {
            $(document).off('keydown.timelineDialog');
            $overlay.remove();
        }
        ...
        if (data.predecessors.length === 0 && !data.taskEndDate) {
            ...
            var $stackInner = $('<div class="timeline-stack-inner">');
            $stack.append($stackInner);
            $stackInner.append($('<div class="timeline-rail"><span class="timeline-rail-arrow"><i class="fa fa-arrow-down"></i></span></div>'));
        ...
        $overlay.on('mousedown', function (ev) {
            if (ev.target === $overlay[0]) closeDialog();
        });
        $(document).on('keydown.timelineDialog', function (ev) {
            if (ev.key === 'Escape') closeDialog();
        });
        $('body').append($overlay);

        // Hint that the list scrolls: if the stack overflows, animate a quick
        // peek to the bottom then back to the top so the user sees there's more.
        setTimeout(function () {
            var el = $overlay.find('.timeline-stack')[0];
            if (!el) return;
    ...
    function buildPredecessorChips($picker, predArray, taskId) {
        ...
        $.each(predArray, function (i, predId) {
            ...
            if (!label) return;
            var pc = getPredColor(i);
            var $chip = $('<span class="pp-chip">').attr('data-pred-id', predId).attr('title', label)
    ...
    function renderPredDropdownList(filterText) {
        ...
        // Determine parent task ID so we can disable it in the list
        var parentTaskId = null;
        if (activePredPickerTarget && activePredPickerTarget.type === 'subtask') {
            var parentInfo = findParentOf(activePredPickerTarget.id, tasks, null);
            if (parentInfo && parentInfo.parent) {
                parentTaskId = parentInfo.parent.id;
            }
        }
        ...
        $.each(labeledRows, function (i, item) {
            ...
            var isDisabled = (parentTaskId !== null && item.pRow.subtask.id === parentTaskId);

            var isParent = (item.pRow.depth === -1);

            var $item = $('<div class="pred-dropdown-item">')
                .attr('data-pred-id', item.pRow.subtask.id)
                .toggleClass('selected', isSelected)
                .toggleClass('pred-disabled', isDisabled)
                .toggleClass('pred-parent-task', isParent);
    ...
    // trigger the handler at design.js:3540. Synthetically trigger `change`
    // (runs the delegated handler synchronously) and then blur, so we don't
    // depend on Edge's native event timing.
