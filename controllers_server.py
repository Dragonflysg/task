...
def _migrate_row_comments_to_tid(data):
    """Convert legacy row-index rowComments keys to tid_<taskId> keys."""
    task_data = data.get('_taskData', {})
    tasks_list = task_data.get('tasks', [])
    if not tasks_list:
        return
    # Build rowIndex -> taskId map
    row_to_tid = {}
    row_idx = [0]
    def _walk(task):
    ...
    for t in tasks_list:
        _walk(t)
    # Remap keys
    old_comments = data.get('rowComments', {})
    new_comments = {}
    needs_migration = False
    for key, val in old_comments.items():
        if str(key).startswith('tid_'):
            new_comments[key] = val
        else:
            try:
                r = int(key)
                tid = row_to_tid.get(r)
                if tid is not None:
                    new_key = 'tid_' + str(tid)
                    if new_key in new_comments:
                        new_comments[new_key] = new_comments[new_key] + val
                    else:
                        new_comments[new_key] = val
                ...
                new_comments[key] = val
    if needs_migration:
        data['rowComments'] = new_comments


def _migrate_celldata_comments_to_tasks(data):
    """Ensure every task object has a 'comments' field.

    We do NOT read from cellData here because after structural changes
    (add/delete subtask) the row-col positions in cellData are stale and
    would map the wrong comment to the wrong task.  The client handles
    seeding task.comments from cellData on first load (when positions are
    still aligned) and keeps it up to date via op:'update' patches.
    """
    task_data = data.get('_taskData', {})
    tasks_list = task_data.get('tasks', [])
    if not tasks_list:
...
def load_group():
    try:
        ...
        if isinstance(data, dict) and data.get('rowComments') and data.get('_taskData'):
            _migrate_row_comments_to_tid(data)
        # Migrate Comments column text from cellData into task objects
        if isinstance(data, dict) and data.get('_taskData'):
            _migrate_celldata_comments_to_tasks(data)
...
def delete_change_request():
    try:
        ...
        if not project or not request_id:
            return jsonify({'ok': False, 'error': 'Missing required fields'}), 400

        data = _load_change_data(project)
        original_len = len(data['requests'])
...
def recompute_ancestor_percents(task_id, tasks_list):
    """Walk the ancestor chain of task_id and recompute each ancestor's
    percentComplete as the rounded average of its direct children.
    Used when a descendant's % (or derived %) changes, so parent roll-ups
    stay persisted without requiring the client to broadcast them.
    """
    chain = []

    def find_chain(lst, target, current):
        for t in lst:
            if t.get('id') == target:
                chain.extend(current)
                return True
            subs = t.get('subtasks') or []
            if subs:
                current.append(t)
                if find_chain(subs, target, current):
                    return True
                current.pop()
        return False

    find_chain(tasks_list, task_id, [])
    # Walk bottom-up: deepest ancestor first, then propagate upward
    for ancestor in reversed(chain):
        subs = ancestor.get('subtasks') or []
        if subs:
            total = 0
            for s in subs:
                try:
                    total += int(s.get('percentComplete') or 0)
                except (TypeError, ValueError):
                    pass
            ancestor['percentComplete'] = round(total / len(subs))


def recompute_ancestor_costs(task_id, tasks_list):
    """Walk the ancestor chain of task_id and recompute each ancestor's
    cost as the sum of its direct children. Used when a leaf cost changes,
    so parent roll-ups stay persisted without requiring the client to
    broadcast them (mirrors recompute_ancestor_percents).
    """
    chain = []
...
def apply_patch_to_file(project, payload):
    ...
    with lock:
        ...
        if op == 'updateCell':
            ...
            task[field] = value
            # Derive % CMPLT from status for leaf tasks — keeps the client
            # broadcast atomic (single field) while still persisting the
            # implied percentComplete value to disk.
            if field == 'status' and value in ('Completed', 'Not Started'):
                subs = task.get('subtasks') or []
                if len(subs) == 0:
                    task['percentComplete'] = 100 if value == 'Completed' else 0
            # Symmetric derivation: % CMPLT → status for leaf tasks.
            # 100 ⇒ Completed; anything below 100 ⇒ In Progress.
            if field == 'percentComplete':
                subs = task.get('subtasks') or []
                if len(subs) == 0:
                    try:
                        pct_int = int(value) if value not in (None, '') else 0
                    except (TypeError, ValueError):
                        pct_int = 0
                    if pct_int >= 100:
            ...
            # Cascade ancestor % CMPLT roll-up when a descendant's %
            # changes (either directly or via status→% derivation).
            # Client broadcasts stay atomic; the server persists the
            # computed parent values so they survive reload.
            if field == 'percentComplete' or (
                field == 'status' and value in ('Completed', 'Not Started')
            ):
                recompute_ancestor_percents(task_id, tasks_list)
            # Cascade ancestor COST roll-up on leaf cost changes, so
            # intermediate parent sums persist across reload without the
            # client broadcasting them explicitly.
            if field == 'cost':
                recompute_ancestor_costs(task_id, tasks_list)
