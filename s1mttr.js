...
function renderKPIs() {
  ...
  const ongoingClass = ongoingNow > 30 ? 'bad' : ongoingNow > 15 ? 'warn' : 'good';

  const cards = [
    { label: 'Median MTTR (window)', value: med.toFixed(1) + 'h', sub: `${resolvedInWindow.length} resolved incidents`, cls: medClass },
    { label: 'Mean MTTR (window)',   value: avg.toFixed(1) + 'h', sub: 'inflated by long tail' },
    { label: '90% resolved within',  value: p90.toFixed(0) + 'h', sub: '10% took longer than this' },
    { label: 'SLA compliance',       value: slaPct.toFixed(1) + '%', sub: `target: ≤${SLA_HOURS}h to resolve`, cls: slaClass },
    { label: 'Incidents in window',  value: inWindow.length, sub: `${resolvedInWindow.length} resolved · ${inWindow.length - resolvedInWindow.length} still open` },
    { label: 'Ongoing right now',    value: ongoingNow, sub: 'across all dates', cls: ongoingClass }
  ];
...
function renderScatter() {
  ...
  mountOrUpdate('scatter', '#scatterChart', {
    chart: { type: 'scatter', height: 400, zoom: { enabled: true, type: 'xy' }, toolbar: { show: false }, animations: { enabled: false } },
    series: [
...
function renderHistogram() {
  ...
  mountOrUpdate('hist', '#histChart', {
    ...
    // green = within SLA, red = breached
    colors: ['#10b981', '#10b981', '#10b981', '#f59e0b', '#ef4444', '#7f1d1d'],
...
function renderSLA() {
  ...
  mountOrUpdate('sla', '#slaChart', {
    chart: { type: 'area', height: 320, toolbar: { show: false }, animations: { enabled: false } },
    ...
    annotations: {
      yaxis: [{ y: 80, borderColor: '#5a6878', strokeDashArray: 4,
        label: { text: '80% target', style: { background: '#5a6878', color: '#fff' } } }]
    },
    grid: { borderColor: '#e5e9f0' }
  });
}
